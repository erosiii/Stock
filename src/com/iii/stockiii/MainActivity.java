...
public class MainActivity extends ActionBarActivity {
	...
	private int posY = 0, top = 0;
	private EditText edtname, edtphone, edtemail;
	private ImageView ivok, ivcancel;
	...
	private void setData() {
		...
		final Runnable r = new Runnable() {
			@Override
			public void run() {
				posY = lsvStock.getFirstVisiblePosition();
	...
	public class WSGetDataDo extends AsyncTask<Void, Void, ArrayList<Stock>> {
		...
		private void setAdapter() {
			...
			adapter = new ListStockAdapter(context, R.layout.item_do_layout,
					lstStock);
			lsvStock.setAdapter(adapter);
			lsvStock.setSelectionFromTop(posY, top);
		}
	}
